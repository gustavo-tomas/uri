...
int main(){
    ...
    int x_fiddle, y_fiddle, x_inv, y_inv, v, r_jump, r_ult;    
    while(cin >> x_fiddle >> y_fiddle >> x_inv >> y_inv >> v >> r_jump >> r_ult){
        double dist = sqrt((x_fiddle-x_inv)*(x_fiddle-x_inv) + (y_fiddle-y_inv)*(y_fiddle-y_inv));
        dist += 1.5*v;

        if(dist > r_jump + r_ult) cout << "N" << endl;
        else cout << "Y" << endl;
    }
